...
function sendReturnOrderRequest(reason,note){
	...
	$.ajax({
		...
		showMessageModalDialog("Return request have been sent");
		updateStatusAndHideReturnButton(response.orderId);
	}).fail(function(err){
		console.log(err);
		showMessageModalDialog("Error Response");
	});
